...
class Bottleneck_1x5(nn.Module):
    expansion = 4

    def __init__(self, in_planes, planes, stride=1, downsample=None):
        super(Bottleneck_1x5, self).__init__()

        self.bn1        = nn.BatchNorm1d(in_planes)
        self.conv1      = conv1x1(in_planes, planes)
        self.bn2        = nn.BatchNorm1d(planes)
        self.conv2      = conv1x5(planes, planes, stride=stride)
        self.bn3        = nn.BatchNorm1d(planes)
        self.conv3      = conv1x1(planes, planes*Bottleneck_1x5.expansion)
        self.downsample = downsample
        # self.strdie = stride
...
class PreActResNet(nn.Module):
    def __init__(self, depth, num_classes_list, dataset, bottleneck=False):
        ...
        if self.dataset.startswith('NIH_Covid'):
            ...
            self.conv1 = conv1x3(1, self.in_planes, stride=2)
            # self.layer0 = self._make_layer(Bottleneck_1x5, 16, 1, stride=2)
            self.layer1 = self._make_layer(block_type, 16, n, stride=1)
